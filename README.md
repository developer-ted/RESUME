# 이력서
[한국어](./README.md) [English](./README_en.md)

<br/>

## 소개
![프로필이미지](https://github.com/developer-ted/RESUME/raw/master/images/profile.jpeg)
- 이름: 김지완
- 생년월일: 1990. 11. 03

<br/>

손으로 10초면 충분히 할 수 있는 일을 <br/>
컴퓨터로 **하루 종일 프로그래밍해서 자동으로 수행**할 때, <br/>
저는 더할 나위 없이 큰 행복을 느낍니다.

<br/>

- 취미: **카페코딩**, 음악, 노래, 다트, 운동, 여행
- Email: it.dev.ted@gmail.com
- GitHub: [github.com/developer-ted](https://github.com/developer-ted)
- LinkedIn: [linkedin.com/in/developer-ted](https://www.linkedin.com/in/developer-ted/)

<br/>

```
If I can make good services, I don't care about anything else.
```


<br/>
<br/>


# 회사경력
## [TEDMONG] (2019. 05 - 현재) <br/>
    현재 TEDMONG에서 다양한 서비스 개발을 맡아서 하고 있습니다.
    
## [Madsquare] [매드스퀘어](http://madsquare.net) (2015. 03 - 2019. 04) <br/>
    매드스퀘어는 승인된 탑 인플루언서들이 활동하는 전용 플랫폼을 구축하여 제품을 판매 하거나 제품을 홍보하는 목표량에 따라, 수익을 쉐어하는 아시아 8개국 최고의 인플루언서 플랫폼인 'CHARIS'를 운영하는 회사입니다.
    개발 팀장을 맡았고, 안드로이드와 아이폰 개발을 맡아서 하였습니다. 
    
## [HandStudio] [핸드스튜디오](http://handstudio.net) (2014. 02 - 2015. 03) <br/>
    핸드스튜디오는 Mobile, WEB, SmartTV 등 다양한 분야의 디지털 미디어 시장을 선도하는 회사입니다. 
    연구원으로 안드로이드 개발을 맡아서 하였습니다
    
## [K&S Company] [케이엔에스] (2013. 01 - 2014. 01)
    모바일 Application을 서비스 기획 및 제작하는 회사입니다. 
    기획 및 서버, 안드로이드 개발을 맡아서 하였고, 대표 역할을 하였습니다


<br/>
<br/>

## 사용 기술
### Android
- Kotlin, JAVA
- Android Architecture Components
- HTTP, Restful API
- MVP, MVVM 등의 디자인 패턴
- DataBinding
- OpenSrouce
- 기타 : glide, picasso, retrofit, realm, firebase, stream...

### iOS
- Swift, Objective-C
- AutoLayout, Constraint, Storyboard
- HTTP, Restful API
- MVP, MVVM 등의 디자인 패턴
- OpenSrouce
- 기타 : cocoapods, realm, alamofire, firebase, afnetworking...

### Process
- 모니터링 - Fabric / Google Analytics / Amplitude
- 커뮤니케이션 - Slack, Jandi
- 이슈관리 - Asana / Github / Jira(애자일/스프린트/칸반) / Trello 
- CI/CD - TravisCI / Coveralls / Jenkins / SonarQube
- 버전관리 - Git (Github / Bitbucket)

### TODO
**새로운것**을 배우는건 언제나 즐거운 일입니다. <br/>
현재는 아래것들을 익히고 있습니다.
- AWS
- Node
- RxSwift
- SwiftUI
- RxJava, RxKotlin
- Android Architecture Components


<br/>
<br/>



# 프로젝트 이력
## 매드스퀘어
### [Farota](https://play.google.com/store/apps/details?id=com.neo.farota)
- 소개 : FAROTA 노지농업, 시설농업에 적용할 수 있는 가장 진보한 관수/관비 솔루션
- 기간 : 2020.01 ~ 2020.01
- 회사명: TEDMONG
- 업무 : Android 앱 개발
- 사용기술 : Kotlin, Android Architecture Components, Retrofit, Firebase 등
- 업무 성과
    - Android 어플리케이션 개발
    - WebApp 서비스 개발


### [시각장애인을 위한 지하철 네비게이션](https://play.google.com/store/apps/details?id=com.hsh.stationalert)
- 소개 : 시각장애인분들을 위한 지하철 네비게이션 애플리케이션
- 기간 : 2019.12 ~ 2020.02
- 회사명: TEDMONG
- 업무 : Android 앱 개발 및 기획,  UI/UX 설계
- 사용기술 : Kotlin, Android Architecture Components, Retrofit, Firebase, Amplitude 등
- 업무 성과
    - 서비스 Project Manager 역할
    - Android 어플리케이션 개발


### [Meething](https://meething.co.kr)
- 소개 : 국내 인플루언서 기반을 두고 제품을 판매하는 샐럽형 커머스 플랫폼
- 기간 : 2019.08 ~ 2019.12
- 회사명: TEDMONG
- 업무 : 서비스 Project Manager 역할
- 업무 성과
    - 서비스 Project Manager 역할


### My Clair
- 소개 : 공기청정기 제어 및 실시간 날씨/미세먼지 알림 서비스 
- 기간 : 2019.06 ~ 2019.08
- 회사명: TEDMONG
- 업무 : iOS 앱 개발 및 기획
- 사용기술 : AFNetworking, Alamofire, Firebase, Database, IOT 등
- 업무 성과
    - iOS 어플리케이션 개발


### [서울대창업지원단](https://apps.apple.com/bm/app/서울대창업지원단/id1472994294)
- 소개 : 
- 기간 : 2019.05 ~ 2019.05
- 회사명: TEDMONG
- 업무 : Android 앱 개발 및 기획,  UI/UX 설계
- 사용기술 : Kotlin, Java, Databinding, Fabric, Retrofit, Firebase, Exoplayer, Channel, Amplitude, AFNetworking, Alamofire,  결제 (BrainTree, Eximbay, Paypal, WechatPay 등), 각종 SNS (Youtube, Instagram, Facebook, Line) 등
- 업무 성과
    - Android 어플리케이션 개발







### HICHARIS 
- 소개 : 동남아시아 8개국 K-Beauty 제품을 각 현지의 인플루언서를 기반으로 샐럽샵과 샐럽의 포스팅을 이용하여 제품을 판매하는 커머스 플렛폼
- 기간 : 2017.10 ~ 2019.04
- 회사명: 매드스퀘어
- 업무 : Android / iOS 앱 개발 및 기획,  UI/UX 설계
- 사용기술 : Kotlin, Java, Databinding, Fabric, Retrofit, Firebase, Exoplayer, Channel, Amplitude, AFNetworking, Alamofire,  결제 (BrainTree, Eximbay, Paypal, WechatPay 등), 각종 SNS (Youtube, Instagram, Facebook, Line) 등
- 업무 성과
    - 결제 및 구매 플로우, 구매 유도를 위한 플로우 등 **커머스 플렛폼 경험**
    - SNS 연동 및 각 SNS 플레이어 적용
    - 서비스 전략 기획 및 UI/UX 설계
    - 3번의 신규 버전 개발 


### CHARIS CELEB 
- 소개 : 동남아시아 8개국 각 현지의 인플루언서들을 위한 커뮤니티 플렛폼
- 기간 : 2016.09 ~ 2019.04
- 회사명: 매드스퀘어
- 업무 : Android / iOS 앱 개발 및 기획,  UI/UX 설계
- 사용기술 : Kotlin, Java, Databinding, Fabric, Retrofit, Firebase, Exoplayer, Channel, Amplitude, AFNetworking, Alamofire, 각종 SNS (Youtube, Instagram, Facebook, Line, Wechat, QQ) 등
- 업무 성과
    - 특정타깃 및 등급별 **커뮤니티 플렛폼 경험**
    - SNS 연동 및 각 SNS 플레이어 적용
    - 서비스 전략 기획 및 UI/UX 설계
    - 4번의 신규 버전 개발


### Humax H5 Aware TV with Snacks
- 소개 : Humax H5 Aware 셋탑박스와 Snack 서비스를 콜라보하는 Android TV Luncher 개발
- 기간 : 2016.04 ~ 2019.09
- 회사명: 매드스퀘어
- 업무 : Android Luncher 개발
- 사용기술 : Java,  Android TV ,Canvas, Realm/SQLite, 각종 서비스 및 애니메이션 기술 사용 등
- 업무 성과
    - **Android TV Luncher 경험**
    - 영상 송수신 기능, LiveApp 및 메모리 최적화 경험
    - 각종 Animation과 Service, DB 사용 경험
    - SNS에서 받아온 영상 플레이어 개발 (WebView, ExoPlayer, MediaPlayer, YoutubePlayer 등)


### 스낵 (Snackk)
- 소개 : 영상을 모아서 나의 채널을 만들고, 사용자들에게 공유할 수 있는 플렛폼에서 영상 추천을 해주는 플렛폼으로 변화  
- 기간 : 2015.01 ~ 2017.01
- 회사명: 매드스퀘어
- 업무 : Android 앱 개발 및 기획,  UI/UX 설계
- 사용기술 : Java, Fabric, Volly, MediaPlayer, Channel, Amplitude, 각종 SNS (Youtube, Instagram, Facebook, Line, Wechat, QQ) 등
- 업무 성과
    - **Android TV Luncher 경험**
    - SNS에서 받아온 영상 플레이어 개발 (WebView, ExoPlayer, MediaPlayer, YoutubePlayer 등)
    - 서비스 전략 기획 및 UI/UX 설계
    - 다양한 UI/UX, 애니메이션 설계 및 구현





### Monkey Age
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 게임 기획 및 유니티를 이용한 게임 개발
- 사용기술 : 
- 업무 성과
    - 게임기획및UI/UX설계
    - 유니티를 이용한 3종 개임 개발
    - 유니티를이용한게임프레임및서버기능구현
    

### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 


### 
- 소개 :  
- 기간 : 2014.04 ~ 2014.05
- 회사명: 핸드스튜디오
- 업무 : 
- 사용기술 : 
- 업무 성과
- 



<br/>
<br/>


## 학력
- 동양미래대학교 전산정보학부 졸업 (2009 ~ 2012)
전문학사, 전산정보학부 4.34/4.5 수석 졸업

<br/>
<br/>


## 단체경력
- 스터디 그룹 'WITHEVER' 활동 (2015 - 현재)
- 스터디 그룹 'DEPS' 활동 (2017 - 2018)
- 개발 동아리 'MARS' 회장으로 활동 (2009 - 2012)


<br/>
<br/>



## 수상 경력

### <SAMSUNG GEAR APP CHALLENGER> 동상 수상
- 수상 날짜 : 2014년 11월
- 시상기관 : SANSUNG
- 설명 : 삼성에서 주최한 <SAMSUNG GEAR APP CHALLENGER>에서 Android 및 Gear 앱 개발을 하였습니다.<br/>2014. 07 ~ 2014. 11 기간 동안 진행이 되었고, 동상을 수상했습니다


### <2012 이브와 프로젝트 멘토링> 지식경제부 장관상 대상 수상
- 수상 날짜 : 2012년 11월
- 시상기관 : 지식경제부, IT 여성 기업인 협회
- 설명 : 지식 경제부, IT 여상 기업인 협회에서 주최한 <2012 이브와 프로젝트 멘토링>에 프로젝트 팀장으로 참가하였습니다.<br/>2012. 04 ~ 2012. 11 기간 동안 진행이 되었고, 지식경제부 장관상 대상을 수상하였습니다.


### <2012 한국 전자전> 장려상 수상
- 수상 날짜 : 2012년 10월
- 시상기관 : 한국 디스플레이 산업 협회, 한국 정보 디스플레이 학회
- 설명 : 킨텍스에서 열리고 한국 디스플레이 산업 협회, 한국 정보 디스플레이 학회에서 주최한 <2012 한국 전자전>에 프로젝트 팀장으로 참가하였습니다. <br/>2012. 03 ~ 2012. 10 기간 동안 진행이 되었고, 장려상을 수상하였습니다.


### <제 6회 이브와 프로젝트 IT 멘토링> 멘토 은상 공로상 수상
- 수상 날짜 : 2013년 11월 
- 시상기관 : 미래 창조 과학부, IT 여상 기업인 협회
- 설명 : 미래 창조 과학부, IT 여상 기업인 협회에서 주최한 <제 6회 이브와 프로젝트 IT 멘토링>에 멘토로 참가하였습니다.<br/>2013. 04 ~ 2013. 11 기간 동안 진행이 되었고, 은상을 수상하였습니다.


### 동양미래대학교 학업 성적 우수상(수석) 수상
- 수상 날짜 : 2013년 2월
- 시상기관 : 동양미래대학교
- 설명 : 동양미래대학교에서 학업 성적 우수상(수석)을 수상하였습니다.


### (주) 효성그룹 대표이사 부회장 공로상 수상
- 수상 날짜 : 2013년 2월
- 시상기관 : 효성그룹, 동양미래대학교
- 설명 : 동양미래대학교 재단인 효성그룹에서 대표이사 부회장 공로상을 수상하였습니다.


### <제 6회 학생 취업 포트폴리오 경진대회> 동상 수상
- 수상 날짜 : 2012년 10월
- 시상기관: 한국 과학 기술 단체 총 연합회, 한국 공학 교육 인증원
- 설명 : 한국 과학 기술 단체 총 연합회, 한국 공학 교육 인증원에서 주최한 <제 6회 학생 취업 포트폴리오 경진대회>에 개인으로 참가하였습니다.<br/>2012. 09 ~ 2012. 10 기간 동안 진행이 되었고, 동상을 수상하였습니다. 


### <2012 창의적 공학설계 경진대회> 우수상 수상
- 수상 날짜 : 2012년 10월
- 시상기관 : 동양미래대학교
- 설명 : 동양미래대학교에서 주최한 <2012 창의적 공학설계 경진대회>에 프로젝트 팀장으로 참가하였습니다.<br/>2012. 03 ~ 2012. 10 기간 동안 진행이 되었고, 우수상을 수상하였습니다.


### <2012 학생 취업 포트폴리오 경진대회> 우수상 수상
- 수상 날짜 : 2012년 10월
- 시상기관 : 동양미래대학교
- 설명 : 동양미래대학교에서 주최한 <2012 학생 취업 포트폴리오 경진대회>에 개인으로 참가하였습니다.<br/>2012. 09 ~ 2012. 10 기간 동안 진행이 되었고, 우수상을 수상하였습니다.


### <2012 스마트 앱(APP) 개발 경진대회> 장려상 수상
- 수상 날짜 : 2012년 11월 
- 시상기관 : 동양미래대학교
- 설명 : 동양미래대학교에서 주최한 <2012 스마트 앱(APP) 개발 경진대회>에 프로젝트 팀장으로 참가하였습니다.<br/>2012. 03 ~ 2012. 11 기간 동안 진행이 되었고, 장려상을 수상하였습니다.


<br/>
<br/>


## 기타 활동
### <캠퍼스 CEO> 2018년도 창업멘토링 / 창업 코칭 프로그램
- 소개 : 2018년도 캠퍼스 CEO 육성사업을 진행하기 위한 창업 멘토링 및 창업 코칭 프로그램에 멘토로 참여
- 기관 : 동양미래대학교 기술창업지원센터
- 기간 : 2018.05 ~ 2018.05

### <캠퍼스 CEO> 2017년도 창업멘토링 / 창업 코칭 프로그램
- 소개 : 2017년도 캠퍼스 CEO 육성사업을 진행하기 위한 창업 멘토링 및 창업 코칭 프로그램에 멘토로 참여
- 기관 : 동양미래대학교 기술창업지원센터
- 기간 : 2017.09 ~ 2017.10

### 동양미래대학교 전산정보학부 동아리 특강 강사
- 소개 : 전산정보학부 스터디 동아리를 참여하고 있는 학생들을 대상으로한 수업에 특강 강사로 참여  
- 기관 : 동양미래대학교
- 기간 : 2016.05 ~ 2016.05

### <캠퍼스 CEO> 2015년도 창업멘토링 / 창업 코칭 프로그램
- 소개 : 2015년도 캠퍼스 CEO 육성사업을 진행하기 위한 창업 멘토링 및 창업 코칭 프로그램에 멘토로 참여
- 기관 : 동양미래대학교 기술창업지원센터
- 기간 : 2015.05 ~ 2015.05

### 동양미래대학교 전산정보학부 특강 강사
- 소개 : 전산정보학부 전 학생을 대상으로 한 특강에 강사로 참여
- 기관 : 동양미래대학교 전산정보학부
- 기간 : 2013.12 ~ 2013.12

### <이브와 프로젝트 IT 멘토링> 기획, UI/UX 및 프로젝트 멘토
- 소개 : ICT 기업인과 공동 프로젝트를 수행하면서 IT 이공계 학생들에게 다양한 산업 현장의 실무를 익힐 수 있는 사업/프로젝트
- 기관 : 미래창조과학부, IT 여성기업인협회
- 기간 : 2013.03 ~ 2013.11

<br/>
<br/>



## 교육 / 연수
### 2013 건국대학교 일반인 창업아카데미 교육 수료
- 소개 : 일반인, 예비창업자 등을 대상으로 실전창업교육, 성공한 CEO의 멘토링 등을 통해 경쟁력 있는 창업자 발굴·육성하는 사업
- 기관 : 건국대학교
- 기간 : 2013.05 ~ 2013.08

### 해외 연수 (미국)
- 소개 : 지식경제부 장관상 대상 부상으로 Google, Microsoft 등 미국 실리콘밸리과 시애틀에 있는 세게 유명기업 및 대학 연수 
- 기관 : 지식경제부
- 기간 : 2012.11 ~ 2012.11

### 아이폰 프로그래밍 교육 수료
- 소개 : 아이폰 개발을 위한 프로그래밍 교육 수료
- 기관 : 동양미래대학교
- 기간 : 2012.12 ~ 2012.12



<br/>
<br/>


----

여기까지 읽어주셔서 감사합니다. <br/>
부족한 부분이나 더 궁금하신 내용이 있다면 [이슈](https://github.com/developer-ted/RESUME/issues)로 남겨주시겨나, it.dev.ted@gmail.com으로 연락주세요.<br/>
감사합니다.
